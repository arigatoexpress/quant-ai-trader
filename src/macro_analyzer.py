class MacroAnalyzer:
    ...
    def analyze(self):
        ...
        if self.macro_data["ten_year_treasury"] > 4.5:
            insights.append(
                "High treasury yields (above 4.5%) may signal risk-off sentiment. Consider favoring BTC over stocks."
            )
        if self.macro_data["inflation"] > 3.5:
            insights.append(
                "Rising inflation (above 3.5%) could favor BTC as an inflation hedge."
            )
        if self.macro_data["global_m2"] > 105e12:
            insights.append(
                "Global liquidity surge (M2 > $105T) may boost risk assets like crypto and stocks."
            )
