...
def calculate_risk_reward(current_price, predicted_price, risk_tolerance):
    potential_gain = (
        abs(predicted_price - current_price) if predicted_price > current_price else 0
    )
...
def plot_price_chart(price_series, prediction, output_path):
    ...
    if prediction is not None:
        plt.scatter(
            price_series.index[-1] + pd.Timedelta(1, unit="d"),
            prediction,
            color="red",
            label="Prediction",
        )
...
def sanitize_filename(name: str) -> str:
    """Return a filename-safe representation of ``name``."""
    import re
